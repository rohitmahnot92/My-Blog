...
app.get('/allPosts', function(req, res) {
  ...
  db.serialize(function() {
    db.each('SELECT Blog.*, Users.* FROM Blog INNER JOIN Users ON Blog.postedBy=Users.uId;', function(err, row) {
      if(err) 
...
app.post('/updateDatabase', function(req, res) {
  ...
  res.contentType('json');
  db.serialize(function() {
    console.log("I AM THR ID" + JSON.stringify(req.body));
    console.log("UPDATE Blog SET title=?, content=?, timePosted=? WHERE pId=?", req.body.title, req.body.content, new Date(), req.body.pId);
    db.run("UPDATE Blog SET title=?, content=?, timePosted=? WHERE pId=?", req.body.title, req.body.content, new Date(), req.body.pId);
    res.send({success:"blog posted"});
  });
});
...
app.post('/updatePost', function(req, res) {
  res.contentType('json');
  db.serialize(function() {
    // db.run("SELECT FROM Blog WHERE pid="+req.body.postId+";");
